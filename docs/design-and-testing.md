...
**Decision:** Deploy on Render, both services on the free plan — no persistent disk.
...
### 2.12 ChromaDB 1.x Compatibility and Startup Data Provisioning

**Decision:** Explicitly call `client.get_or_create_collection()` before constructing the LangChain `Chroma` wrapper, and download a pre-built ChromaDB archive from a GitHub Release asset on container startup rather than relying on a persistent disk.

**Reason (API compatibility):** ChromaDB 1.x changed collection-access behavior — the LangChain `Chroma` wrapper's default path calls `get_collection()` internally, which raises a `NotFoundError` if the collection doesn't already exist. This surfaced as a `RetryError[NotFoundError]` at query time in production, even though ingestion had completed successfully locally, because the query-side and ingestion-side code paths were not guaranteed to initialize the collection consistently.

**Reason (no persistent disk):** Render's free tier does not support persistent disks; a paid Starter/Standard plan would cost ~$9.50/month just to store ~18 documents' worth of vectors. Since the app's raw PDF documents are gitignored (never reach the Docker build context regardless of disk), ingestion is instead treated as a one-time local step. Its *output* — the `chroma_db/` folder — is zipped and uploaded as a GitHub Release asset, then downloaded and extracted into the container's local filesystem on every cold start, before the app begins serving requests.

**Implementation:** `backend/app/services/retriever_service.py` and `backend/ingestion/indexer.py` both construct an explicit `chromadb.PersistentClient`, call `get_or_create_collection("greenlens_docs")`, and pass that client into `Chroma(client=..., ...)`. `backend/app/startup.py` handles the download-and-extract logic, called from `main.py`'s FastAPI lifespan handler before the app accepts requests. This is a no-op if `CHROMA_PATH` already contains data (e.g. local dev).
